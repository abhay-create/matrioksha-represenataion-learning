...
print(f"Standard shape: {std_emb.shape}, MRL-v4 shape: {mrl_emb.shape}")
V = std_emb.shape[0]
...
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "exp1_mean_sim_v4.png"))
plt.close()
...
for d in levels:
    ...
    print(f"Level {d:<3}: Avg Cos Sim (Std, Mrl_mean) = {np.mean(cs1):.4f}, (Mrl, Std_mean) = {np.mean(cs2):.4f}")

plt.figure(figsize=(8,5))
plt.plot(levels, std_on_mrl_mean, marker='o', label="Std words with MRL-v4 mean")
plt.plot(levels, mrl_on_std_mean, marker='s', label="MRL-v4 words with Std mean")
plt.title("Avg Cross Cosine Similarity across Prefixes (V4)")
plt.xlabel("Prefix Dimension")
plt.ylabel("Mean Cosine Similarity")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "exp2_cross_sim_v4.png"))
plt.close()
...
plt.plot(np.cumsum(pca_mrl.explained_variance_ratio_), label="MRL-v4 300d", color='orange')
plt.title("Cumulative Explained Variance (Top 50 PCs) - Full Vocab")
plt.xlabel("Principal Component")
plt.ylabel("Cumulative Explained Variance Ratio")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "exp3_pca_full_v4.png"))
plt.close()
print(f"Top 50 PCs EVR -> Std: {np.sum(pca_std.explained_variance_ratio_):.4f}, MRL-v4: {np.sum(pca_mrl.explained_variance_ratio_):.4f}")
...
for i, (b_name, (start, end)) in enumerate(buckets.items()):
    ...
    axs[i].plot(np.cumsum(pca_mrl.explained_variance_ratio_), label="MRL-v4 300d", color='orange')
    ...
    print(f"Bucket {b_name} Top {n_comp} PCs EVR -> Std: {np.sum(pca_std.explained_variance_ratio_):.4f}, MRL-v4: {np.sum(pca_mrl.explained_variance_ratio_):.4f}")

plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "exp4_pca_buckets_v4.png"))
plt.close()
...
for i in range(len(pairs)-1):
    ...
    print(f"Pair {s1:03}-{e1:03} vs {s2:03}-{e2:03} -> Cos Sim | Std: {sim_std:6.4f}, MRL-v4: {sim_mrl:6.4f}")
...
rects2 = ax.bar(x + width/2, mrl_angles, width, label='MRL-v4', color='orange')
ax.set_ylabel('Cosine Similarity')
ax.set_title('Cosine Similarity between Consecutive 50d Chunks of Mean Vector (V4)')
ax.set_xticks(x)
ax.set_xticklabels(labels, rotation=45)
ax.legend()
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "exp5_consecutive_angles_v4.png"))
plt.close()
...
print(f"Avg Cos Sim to Mean | Std300: {np.mean(std_sims_300):.4f}, Std50: {np.mean(std_sims_50):.4f}, MRL-v4 300: {np.mean(mrl_sims_300):.4f}, MRL-v4 50: {np.mean(mrl_sims_50):.4f}")

plt.figure(figsize=(10,6))
plt.hist(std_sims_300, bins=50, alpha=0.5, label='Std 300d', density=True)
plt.hist(mrl_sims_300, bins=50, alpha=0.5, label='MRL-v4 300d', density=True)
plt.hist(mrl_sims_50, bins=50, alpha=0.5, label='MRL-v4 50d', density=True)
plt.title("Distribution of Cosine Sim to Mean Vector (The Narrow Cone V4)")
plt.xlabel("Cosine Similarity to Mean")
plt.ylabel("Density")
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "exp6a_cone_alignment_v4.png"))
plt.close()

std_pos_pct = np.mean(std_emb > 0, axis=0)
mrl_pos_pct = np.mean(mrl_emb > 0, axis=0)

print(f"Avg % Positive Components overall | Std: {np.mean(std_pos_pct)*100:.2f}%, MRL-v4: {np.mean(mrl_pos_pct)*100:.2f}%")

plt.figure(figsize=(10,5))
plt.plot(np.arange(1, 301), std_pos_pct, label="Standard", alpha=0.7)
plt.plot(np.arange(1, 301), mrl_pos_pct, label="MRL-v4", alpha=0.7)
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(plot_dir, "exp6b_non_negativity_v4.png"))
plt.close()
...
for d in levels:
    ...
    print(f"Level {d:<3} Avg L2 | Top 100 Std: {norm_std_res['Top 100'][-1]:.2f}, MRL-v4: {norm_mrl_res['Top 100'][-1]:.2f}")

fig, axs = plt.subplots(1, 4, figsize=(20,5))

for i, b_name in enumerate(buckets.keys()):
    axs[i].plot(levels, norm_std_res[b_name], marker='o', label="Standard")
    axs[i].plot(levels, norm_mrl_res[b_name], marker='s', label="MRL-v4")
    axs[i].set_title(b_name)
...
axs[3].plot(levels, avg_norm_mrl_full, marker='s', label="MRL-v4")
...
plt.tight_layout()
plt.savefig(os.path.join(plot_dir, "exp7_l2_norm_freq_v4.png"))
plt.close()
